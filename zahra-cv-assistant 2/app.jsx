...
Zahra's answer (in ${langName}):`;
  ...
  // 2) Serverless proxy (/api/ask) — your Anthropic key stays on the server,
  //    so visitors never see it and never have to sign in.
  let r;
  try{
    ...
    throw new Error("Pas de connexion au service IA. Cette app doit tourner sur un hébergeur avec fonctions (Vercel), pas en fichier local.");
  }
  if(r.status === 404){
    throw new Error("Fonction /api/ask introuvable (404). Sur Vercel, mets le dossier 'api' à la racine du déploiement (Root Directory), puis redéploie.");
  }
  if(!r.ok){
...
function App(){
  ...
  const runVoice = async (text) => {
    ...
    try{
      ...
      setMessages(prev => [...prev, { role: "zahra", text: t.answerErr + (detail ? "\n\n(" + detail + ")" : "") }]);
      showToast(detail || t.answerErr);
    }
  };
